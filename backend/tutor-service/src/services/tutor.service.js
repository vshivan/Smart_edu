...
const listTutors = async (query) => {
  ...
  if (subject)    { conditions.push(`$${i++} = ANY(tp.skills)`); params.push(subject); }
  if (min_rating) { conditions.push(`tp.rating >= $${i++}`);     params.push(parseFloat(min_rating)); }
  if (max_rate)   { conditions.push(`tp.hourly_rate <= $${i++}`); params.push(parseFloat(max_rate)); }
  if (search)     { conditions.push(`(u.first_name ILIKE $${i++} OR u.last_name ILIKE $${i++} OR tp.bio ILIKE $${i++})`); params.push(`%${search}%`, `%${search}%`, `%${search}%`); i += 2; }
  ...
  const count = await pool.query(`SELECT COUNT(*) FROM tutor_profiles tp WHERE ${where}`, params);
...
const bookSession = async (learnerId, { tutor_id, slot_id, subject, notes }) => {
  ...
  const { rows } = await pool.query(
    ...
    [tutorUserId, learnerId, slot_id, subject, slot.rows[0].slot_start, duration, amount, platformFee, tutorEarnings, notes]
  );
  return rows[0];
};
...
module.exports = { listTutors, getTutorProfile, updateAvailability, addSlots, getAvailableSlots, bookSession, updateSessionStatus, getTutorEarnings };
