...
const getUsers = async (query) => {
  ...
  if (is_banned !== undefined) { conditions.push(`is_banned = $${i++}`); params.push(is_banned === 'true'); }
  if (search)     { conditions.push(`(email ILIKE $${i++} OR first_name ILIKE $${i++} OR last_name ILIKE $${i++})`); params.push(`%${search}%`, `%${search}%`, `%${search}%`); i += 2; }
  ...
  const { rows } = await pool.query(
    `SELECT id, email, first_name, last_name, role, is_active, is_banned, ban_reason, last_login_at, created_at
     FROM users WHERE ${where} ORDER BY created_at DESC LIMIT $${i++} OFFSET $${i++}`,
    [...params, limit, offset]
  );
...
const getAnalytics = async (period = '30d') => {
  ...
  const [signups, enrollments, quizStats, revenue] = await Promise.all([
    ...
    pool.query(`
      ...
      FROM enrollments WHERE enrolled_at > NOW() - INTERVAL '${interval}'
